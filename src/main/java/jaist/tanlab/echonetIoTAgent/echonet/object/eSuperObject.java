 ...
 *
 * Unless required by thislicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  See the
 * License for the specific language governing permissions and limitations under
 * the License.
 ******************************************************************************/
package jaist.tanlab.echonetIoTAgent.echonet.object;
...
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x80Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x81Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x82Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x83Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x84Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x85Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x86Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x87Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x88Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x89Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8AHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8BHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8CHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8DHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8EHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x8FHandler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x93Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x97Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x98Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x99Handler;
import jaist.tanlab.echonetIoTAgent.echonet.object.epchandlers.EPC0x9AHandler;
...
public class eSuperObject {
	...
	public ServiceExecutor cmdExecutor;
	private String id;
	public boolean setOperationStatusServiceRegistered = false;
	public boolean setInstallationServiceRegistered = false;
	public boolean setCurrentLimitServiceRegistered = false;
	public boolean setPowerSavingOperationServiceRegistered = false;
	public boolean setRemoteControlSettingServiceRegistered = false;
	public boolean setCurrentDateServiceRegistered = false;
	public boolean setCurrentTimeServiceRegistered = false;
	public boolean setPowerLimitServiceRegistered = false;
	/**
	 * Device IP
	 */
	private Node node;
	...
	public eSuperObject(Node node, EOJ eoj, boolean observable) {
		eSuperObject.this.node = node;
		eSuperObject.this.eoj = eoj;
		eSuperObject.this.deviceID = node.getNodeInfo().toString().replace(".", "_")+"_"+eoj.getInstanceCode();
		eSuperObject.this.type = eDeviceType.SuperObject;
		this.observeEnabled = observable;
	}
	...
	public boolean setCurrentLimitSetting(Node node, EOJ eoj, int limitSetting) {
		...
		if(getCurrentLimitSetting() == limitSetting) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x87, new ObjectData(Integer.valueOf(limitSetting).byteValue()))) {
	...
	public boolean setpowerSavingMode(Node node, EOJ eoj) {
		...
		if(getPowerSaving()) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x8F, new ObjectData((byte) 0x41))) {
				EPC0x8FHandler.refreshPowerSaving(eSuperObject.this,true);
				rs= true;
			} else {
				rs = false;
	...
	public boolean turnOffpowerSavingMode(Node node, EOJ eoj) {
		...
		if(!getPowerSaving()) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x8F, new ObjectData((byte) 0x42))) {
				EPC0x8FHandler.refreshPowerSaving(eSuperObject.this,false);
				rs= true;
			} else {
				rs = false;
	...
	public boolean setThroughPublicNetwork(Node node, EOJ eoj) {
		...
		if(getThroughPublicNetwork()) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x93, new ObjectData((byte) 0x42))) {
				EPC0x93Handler.refreshThroughPublicNetwork(eSuperObject.this,true);
				rs= true;
			} else {
				rs = false;
	...
	public boolean setNotThroughPublicNetwork(Node node, EOJ eoj) {
		...
		if(!getThroughPublicNetwork()) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x93, new ObjectData((byte) 0x41))) {
				EPC0x93Handler.refreshThroughPublicNetwork(eSuperObject.this,false);
				rs= true;
			} else {
				rs = false;
	...
	public boolean setPowerLimitSetting(Node node, EOJ eoj,short powerLimitSetting) {
		...
		if(getPowerLimit() == powerLimitSetting) {
			...
			if(this.cmdExecutor.executeCommand(node,eoj,EPC.x99, new ObjectData(Integer.valueOf(powerLimitSetting).byteValue()))) {
	...
	public boolean setOn() {
		...
		if(getOperationStatus()) {
			...
			if(this.cmdExecutor.executeCommand(this.node,this.eoj,EPC.x80, new ObjectData((byte) 0x30))) {
				EPC0x80Handler.refreshOperationStatus(eSuperObject.this,true);
				rs= true;
			} else {
				rs = false;
		...
		Long lightOnTime = System.currentTimeMillis() - startTime;
		System.out.println("lightOnTime:" + lightOnTime);
		return rs;
	}
	public boolean setOff() {
		...
		if(!getOperationStatus()) {
			...
			if(this.cmdExecutor.executeCommand(this.node,this.eoj,EPC.x80, new ObjectData((byte) 0x31))) {
				EPC0x80Handler.refreshOperationStatus(eSuperObject.this,false);
				rs= true;
			} else {
				rs = false;
		...
		Long lightOffTime = System.currentTimeMillis() - startTime;
		System.out.println("lightOffTime:" + lightOffTime);
		return rs;
	}
