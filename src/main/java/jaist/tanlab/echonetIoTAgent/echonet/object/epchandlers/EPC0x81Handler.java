...
public class EPC0x81Handler {
	...
	public static final String description = "Service to change the installation location of a device";
		...
		if(obj instanceof eSuperObject) {
			...
			if(type!= eDeviceType.Profile) {
				if(!newVal.equals(dev.getInstallLocation())) {
					if(!((eSuperObject) obj).setInstallationServiceRegistered) {
						NGSIUtils.registerService(node, eoj, type, description, Arrays.asList("InstallationLocation"));
						((eSuperObject) obj).setInstallationServiceRegistered = true;
					}
					dev.setInstallLocation(newVal);
					NGSIUtils.addAttributeToEntity(node, eoj, type, "InstallationLocation", new FiwareAttribute(newVal));
