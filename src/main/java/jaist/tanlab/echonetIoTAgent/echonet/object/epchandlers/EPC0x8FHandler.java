...
public class EPC0x8FHandler {
	...
	public static final String description = "Service to set power saving operation setting of a device";
	public static void refreshPowerSaving(Object obj, boolean newVal){
		...
		if(obj instanceof eSuperObject) {
			...
			if(dev.getPowerSaving() != newVal) {
				if(!((eSuperObject) obj).setPowerSavingOperationServiceRegistered) {
					NGSIUtils.registerService(node, eoj, type, description, Arrays.asList("PowerSavingOperationSetting"));
					((eSuperObject) obj).setPowerSavingOperationServiceRegistered = true;
					
				}
				dev.setPowerSaving(newVal);
				NGSIUtils.addAttributeToEntity(node, eoj, type, "PowerSavingOperationSetting", 
