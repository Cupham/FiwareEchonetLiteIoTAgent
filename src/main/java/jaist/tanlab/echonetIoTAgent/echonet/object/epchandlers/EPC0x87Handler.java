...
public class EPC0x87Handler {
	...
	public static final String description = "Service to set current limit setting of a device";
		...
		if(obj instanceof eSuperObject) {
			...
			if(type!= eDeviceType.Profile) {	
				if(dev.getCurrentLimitSetting() != newVal) {
					if(!((eSuperObject) obj).setCurrentLimitServiceRegistered) {
						NGSIUtils.registerService(node, eoj, type, description, Arrays.asList("CurrentLimitSetting"));
						((eSuperObject) obj).setCurrentLimitServiceRegistered = true;
					}
					dev.setCurrentLimitSetting(newVal);
					NGSIUtils.addAttributeToEntity(node, eoj, type, "CurrentLimitSetting", 
